...
Our harness will provide timing results for `hcubature` and
`pcubature` (where appropriate). We begin by creating a harness for
these calls.
...
The package `R2Cuba` is no longer available on CRAN. Fear not, for
[version 2.0](https://bnaras.github.io/cubature) integrates the latest
version (4.2) of the [Cuba
libraries](https://directory.fsf.org/wiki/Cuba) (canonical web link is
not secure) with R using `Rcpp`. In fact, you can install the
development version from the master branch of my [Github
repo](https://github.com/bnaras/cubature). Vectorization may be used
with these routiness too.
