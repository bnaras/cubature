...
globally adaptive subdivision scheme. The subdivision algorithm is
similar to [`suave()`](suave.md).
...
- f:
  ...
  The function (integrand) to be integrated. For cuhre, it can be
  something as simple as a function of a single argument, say x.
...
- key:
  ...
  the quadrature rule key: `key = 7, 9, 11, 13` selects the cubature
  rule of degree key. Note that the degree-11 rule is available only in
  3 dimensions, the degree-13 rule only in 2 dimensions. For other
  values, including the default 0, the rule is the degree-13 rule in 2
...
Mathematical Software*, **17**(4), 437-451.

T. Hahn (2005) CUBA-a library for multidimensional numerical
integration. *Computer Physics Communications*, **168**, 78-95. See
