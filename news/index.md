...
- New `robust = TRUE` argument for
  [`hcubature()`](../reference/hcubature.md),
  [`pcubature()`](../reference/hcubature.md), and
  [`cubintegrate()`](../reference/cubintegrate.md), opting in to more
  conservative error estimation for non-smooth or localized integrands.
  Default behavior is unchanged; see the Get Started vignette for
  details and limitations.
- **Breaking change**: [`hcubature()`](../reference/hcubature.md) and
  [`pcubature()`](../reference/hcubature.md) now return `returnCode = 2`
  (`CUBATURE_NOT_CONVERGED`) and emit a warning when `maxEval` is
  exhausted without reaching the requested tolerance. Previously such
  cases silently returned `returnCode = 0`. New C-header constants
  `CUBATURE_SUCCESS`, `CUBATURE_FAILURE`, and `CUBATURE_NOT_CONVERGED`
  are exported.
- Vignettes reorganized: `cubature.Rmd` is now a gentle “Get started”
  introduction covering [`cubintegrate()`](../reference/cubintegrate.md)
  and the robust path; benchmark content moved to a new
  `vectorization.Rmd` vignette. The `version2.Rmd` notes have been
  retired.
- New hex sticker (`man/figures/logo.png`) and pkgdown theme.
- **Breaking change**: Removed deprecated `doChecking` parameter from
  [`hcubature()`](../reference/hcubature.md) and
  [`pcubature()`](../reference/hcubature.md) (ignored since v2.0).
- Fixed [`default_args()`](../reference/default_args.md) typo (`sauve` →
  `suave`) and [`cubintegrate()`](../reference/cubintegrate.md) doc typo
  (`forcCuba` → `for Cuba`).
- Harmonized default `absError` between `hcubature` and `pcubature`
  (both now `.Machine$double.eps * 10^2`).
...
- Build-system fix: `.ts` timestamp files now depend on the C sources,
  so edits trigger rebuilds correctly.
- Added a comprehensive test suite covering input validation, error
  handling, boundary conditions, method consistency, and regression
...
- Change default `absError` value in `hcubature`, `adaptIntegrate`,
  `pcubature` to `.Machine$double.eps * 10^2` to avoid nontermination, a
  breaking change unfortunately, but necessary
- Address issue raised by [@jeroen](https://github.com/jeroen) about
  cross-compilation (thanks Jeroen).
...
- Added RANLIB call to `libcubature.a` ([Issue
  40](https://github.com/bnaras/cubature/issues/40)). Thanks Sergei
  Fedorov.
- Simplified windows/non-windows organization, with default being
  windows
...
- Bug fix: increment correct count of number of evaluations. Thanks Jan
  Meis.
...
- Fix math typesetting in documentation and use roxygen markdown tags
  wherever possible.
...
- Fix unprotected arguments ([Issue
  34](https://github.com/bnaras/cubature/issues/34)). Thanks Manuel
  Koller.
- Fix documentation ([Issue
  32](https://github.com/bnaras/cubature/issues/32)). Thanks Emanuele
  Guidotti.
...
- Fix `Makevars` to refer to `CC` not `C` ([Issue
  33](https://github.com/bnaras/cubature/issues/33))
- Replace `src/Cuba-4.2-win/common/Random.c:105` with equivalent to
  ensure gcc 8.x erroneous warning is avoided on Windows
...
- Fixed up uninitialized count for `hcubature` (Thanks, Ehsan Masoudi)
...
- Miscellaneous fixes for compilation on various platforms.
  ...
  - Reworked code around `Cuba-4.2/src/divonne/Rule.c` checks for key
    values (lines 593 to 600).
  - Fixed up call to `WORKER` variadic macro by including dummy argument
    0 in `Cuba-4.2/src/common/Parallel.c`, line 393.
  - Moved `#ifdef FRAMECOPY` outside in `Cuba-4.2/src/common/Parallel.c`
    to avoid embedding directives in macro args, lines 70-80.

## cubature 2.0

CRAN release: 2018-11-23

- Major update. Integrates [Cuba 4.2](https://feynarts.de/cuba/) library
- Allows for finite or infinite limits of integration (courtesy of Simon
  Gaure)
...
- Fixed up the C call so that it is re-entrant (brought to my attention
  by Pierre de Villemereuil). This should be considered a bug fix!
...
- Added LICENSE file ([request of Nick
  Youngblut](https://github.com/bnaras/cubature/issues/12))
...
- Merged Manuel Koller’s registration for C code and vignette fix for
  NA.
- Moved cubature header and exp_cubature headers to `inst/include` for
  linking to other packages

## cubature 1.3-10

- Renamed `Readme.md` to `README.md`
- Removed references to ab-initio website that caused some hassle due to
  misconfigured site.
...
- Registered .Call stuff and removed microbenchmark suggestion in favor
  of benchr.

## cubature 1.3-4

- Generated smaller pcubature header (`clencurt.h`) using M = 16 and put
  back C cubature source in tree.

## cubature 1.3-3

- Moved the cubature-1.0.2 C library to github to avoid hitting the CRAN
  size limit.
...
- `hcubature` function gains a norm argument that is set to a sensible
  default so as not to affect depending packages
...
- Roxygenized in preparation for upgrade to newer version of cubature
  library on abinitio website
...
- Added `doChecking` argument (default `FALSE`) to save some computation
  time in evaluating integrand (9% speedup).
...
- Corrected radius constant in `testFn2` to match cubature output
  exactly
