...
{
  number seq = t->rng.rng_union.sobol.seq++;
  count zerobit = 0, dim;
  ...
  for( dim = 0; dim < t->ndim; ++dim ) {
    t->rng.rng_union.sobol.prev[dim] ^= t->rng.rng_union.sobol.v[dim][zerobit];
    x[dim] = t->rng.rng_union.sobol.prev[dim]*t->rng.rng_union.sobol.norm;
  }
}
...
static inline void SobolIni(This *t)
{
  ...
  t->rng.rng_union.sobol.norm = ldexp(.5, -nbits);

  for( bit = 0; bit <= nbits; ++bit )
    t->rng.rng_union.sobol.v[0][bit] = (number)1 << (nbits - bit);

  for( dim = 1; dim < t->ndim; ++dim ) {
    number *pv = t->rng.rng_union.sobol.v[dim], *pvv = pv;
  ...
  t->rng.rng_union.sobol.seq = 0;
  XClear(t->rng.rng_union.sobol.prev);
...
{
  count next = t->rng.rng_union.mersenne.next, dim;
  ...
  for( dim = 0; dim < t->ndim; ++dim ) {
    ...
    if( next >= MERSENNE_N ) {
      MersenneReload(t->rng.rng_union.mersenne.state);
      next = 0;
    }

#if RANDOM_BITS == 53
    a = MersenneInt(t->rng.rng_union.mersenne.state[next++]) >> 5;
    b = MersenneInt(t->rng.rng_union.mersenne.state[next++]) >> 6;
    x[dim] = (67108864.*a + b)/9007199254740992.;
#else
    x[dim] = MersenneInt(t->rng.rng_union.mersenne.state[next++])/4294967296.;
#endif
  }

  t->rng.rng_union.mersenne.next = next;
}
...
{
#if RANDOM_BITS == 53
  n = 2*n*t->ndim + t->rng.rng_union.mersenne.next;
#else
  n = n*t->ndim + t->rng.rng_union.mersenne.next;
#endif
  t->rng.rng_union.mersenne.next = n % MERSENNE_N;
  n /= MERSENNE_N;
  while( n-- ) MersenneReload(t->rng.rng_union.mersenne.state);
}
...
{
  state_t seed = t->seed;
  state_t *next = t->rng.rng_union.mersenne.state;
  count j;
  ...
  MersenneReload(t->rng.rng_union.mersenne.state);
  t->rng.rng_union.mersenne.next = 0;
...
static inline int RanluxInt(This *t, count n)
{
  ...
  while( n-- ) {
    s = t->rng.rng_union.ranlux.state[t->rng.rng_union.ranlux.j24] -
        t->rng.rng_union.ranlux.state[t->rng.rng_union.ranlux.i24] + t->rng.rng_union.ranlux.carry;
    s += (t->rng.rng_union.ranlux.carry = NegQ(s)) & (1 << 24);
    t->rng.rng_union.ranlux.state[t->rng.rng_union.ranlux.i24] = s;
    --t->rng.rng_union.ranlux.i24;
    t->rng.rng_union.ranlux.i24 += NegQ(t->rng.rng_union.ranlux.i24) & 24;
    --t->rng.rng_union.ranlux.j24;
    t->rng.rng_union.ranlux.j24 += NegQ(t->rng.rng_union.ranlux.j24) & 24;
  }
...
/* The Generator proper: "Subtract-with-borrow",
  ...
  for( dim = 0; dim < t->ndim; ++dim ) {
    cint nskip = (--t->rng.rng_union.ranlux.n24 >= 0) ? 0 :
      (t->rng.rng_union.ranlux.n24 = 24, t->rng.rng_union.ranlux.nskip);
    cint s = RanluxInt(t, 1 + nskip);
    x[dim] = ldexp(s, -24);
/* small numbers (with less than 12 significant bits) are "padded" */
    if( s < (1 << 12) )
      x[dim] += ldexp(t->rng.rng_union.ranlux.state[t->rng.rng_union.ranlux.j24], -48);
  }
}
...
{
  RanluxInt(t, n + t->rng.rng_union.ranlux.nskip*(n/24));
  t->rng.rng_union.ranlux.n24 = 24 - n % 24;
}
...
static inline void RanluxIni(This *t)
{
  ...
  t->rng.rng_union.ranlux.nskip = level - 24;

  t->rng.rng_union.ranlux.i24 = 23;
  t->rng.rng_union.ranlux.j24 = 9;
  t->rng.rng_union.ranlux.n24 = 24;

  for( i = 0; i < 24; ++i ) {
    cint k = seed/53668;
    seed = 40014*(seed - k*53668) - k*12211;
    seed += NegQ(seed) & 2147483563;
    t->rng.rng_union.ranlux.state[i] = seed & ((1 << 24) - 1);
  }

  t->rng.rng_union.ranlux.carry = ~TrueQ(t->rng.rng_union.ranlux.state[23]) & (1 << 24);
