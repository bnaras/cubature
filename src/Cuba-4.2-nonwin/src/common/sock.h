...
static inline int readsock(cint fd, void *data, csize_t n)
{
  ssize_t got;
  size_t remain = n;
  unsigned char* cur_ptr = (unsigned char*) data;
  do got = recv(fd, (void *) cur_ptr, remain, MSG_WAITALL);
  while( got > 0 && (cur_ptr += got, remain -= got) > 0 );
...
static inline int writesock(cint fd, const void *data, csize_t n)
{
  ...
  size_t remain = n;
  unsigned char* cur_ptr = (unsigned char*) data;
  do got = send(fd, (void *) cur_ptr, remain, MSG_WAITALL);
  while( got > 0 && (cur_ptr += got, remain -= got) > 0 );
